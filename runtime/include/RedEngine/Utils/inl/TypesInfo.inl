...
constexpr TypeTraits TypeInfo()
{
    ...
    //static_assert(left_marker_index != StringView::npos, "Type T is not starting with left_marker");
    ...
    //static_assert(end_index != StringView::npos, "Type T is not ending with right_marker");

    constexpr auto length = end_index - start_index;

    constexpr auto typeName = full_name.substr(start_index, length);

    constexpr auto structSlicedTypeName = function_struct_start.empty() == false && typeName.find(function_struct_start) != StringView::npos
                                              ? typeName.substr(function_struct_start.length())
                                              : typeName;

    constexpr auto classSlicedTypeName = function_class_start.empty() == false && structSlicedTypeName.find(function_class_start) != StringView::npos
